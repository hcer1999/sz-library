...
function getAptitudeColor(level: number) {
  switch(level) {
    case 4: return 'text-yellow-500 font-bold' // S
    case 3: return 'text-red-400 font-semibold' // A
    case 2: return 'text-blue-500' // B
    case 1: return 'text-gray-500' // C
    default: return 'text-gray-400' // 未知
  }
}
...
export function HeroCard({ hero, onClick }: HeroCardProps) {
  const getCountryStyle = (country: number): { bgColor: string, hoverBgColor: string, borderColor: string } => {
    const styles = {
      0: { // 魏国
        ...
        <div className="flex-1 p-4 space-y-3">
          ...
          <div className="grid grid-cols-5 gap-2 text-sm">
            <div className="text-center">
              ...
              <div className={getAptitudeColor(hero.cavalry)}>
                {getAptitudeText(hero.cavalry)}
              </div>
            </div>
            <div className="text-center">
              <div className="text-muted-foreground text-xs">盾兵</div>
              <div className={getAptitudeColor(hero.shield)}>
                {getAptitudeText(hero.shield)}
              </div>
            </div>
            <div className="text-center">
              <div className="text-muted-foreground text-xs">弓兵</div>
              <div className={getAptitudeColor(hero.archers)}>
                {getAptitudeText(hero.archers)}
              </div>
            </div>
            <div className="text-center">
              <div className="text-muted-foreground text-xs">枪兵</div>
              <div className={getAptitudeColor(hero.spearman)}>
                {getAptitudeText(hero.spearman)}
              </div>
            </div>
            <div className="text-center">
              <div className="text-muted-foreground text-xs">器械</div>
              <div className={getAptitudeColor(hero.mechanical)}>
                {getAptitudeText(hero.mechanical)}
              </div>
            </div>
          </div>
